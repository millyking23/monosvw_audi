  ...
  if (status === "success") {
    return (
      <div className="form-shell text-center py-10">
        ...
        <h3 className="text-xl font-display font-semibold text-white">Quote request received</h3>
    ...
    <div className="form-shell" id="quote">
      <form ref={formRef} onSubmit={handleSubmit}>
        ...
        <div className="grid sm:grid-cols-2 gap-5">
          ...
          <div className="field sm:col-span-2"><label>Email (optional)</label><input name="email" type="email" placeholder="you@email.com" /></div>
          <div className="field sm:col-span-2"><label>Vehicle / Machine</label><input required name="vehicle" type="text" placeholder="e.g. VW Golf 7, 2018" /></div>
          <div className="field sm:col-span-2"><label>Describe the Issue</label><textarea required name="issue" rows={4} placeholder="Tell us what's wrong, any noises, warning lights, or damage..." /></div>
          <div className="field sm:col-span-2">
            <label>Upload Photos</label>
            <div className={`upload-drop ${dragOver ? "drag" : ""}`} onClick={() => fileInputRef.current?.click()} onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={(e) => { e.preventDefault(); setDragOver(false); if (e.dataTransfer.files.length && fileInputRef.current) { fileInputRef.current.files = e.dataTransfer.files; setFileCount(e.dataTransfer.files.length); } }}>
        ...
        <div className="flex items-center gap-4 flex-wrap mt-7">
          <button type="submit" className="btn btn-primary" disabled={status === "loading"}>{status === "loading" && <span className="spinner" />}{status === "loading" ? "Sending..." : "Send Quote Request"}</button>
          <span className="text-[0.78rem] text-silver-dim">Quotations are typically returned within 24 hours.</span>
        </div>
      </form>
    </div>
  );
